(*
Darpan Choudhary || dc3292
Xiao Lim || xl2669
Zion Lin || zel2109
Katherine Patton || kyp2106 
*)
(* Semantically-checked Abstract Syntax Tree and functions for printing it *)

open Ast

type sexpr = typ * sx
and sx =
    SIntlit of int
  | SFliteral of string
  | SCharlit of char
  | SStrlit of string
  | SBoolLit of bool
  | SArraylit of sexpr list * int (* list of expressions and size of array *)
  | SArraccess of sexpr * sexpr
  | SId of string
  | SBinop of sexpr * op * sexpr
  | SUnop of uop * sexpr
  | SIncDec of sexpr * incdec
  | SAssign of sexpr * sexpr
  | SSretrieve of sexpr * string
  | SCall of string * sexpr list
  | SNoexpr


type sstmt =
    SBlock of bind list * sstmt list
  | SExpr of sexpr
  | SReturn of sexpr
  | SIf of sexpr * sstmt * sstmt
  | SFor of sexpr * sexpr * sexpr * sstmt
  | SWhile of sexpr * sstmt

type sfunc_decl = {
    styp : typ;
    sfname : string;
    sformals : bind list;
    slocals : bind list;
    sbody : sstmt list;
  }


type sstruct_decl = {
  ssname : string;
  smembers : bind list;
}

type sprogram = bind list * sfunc_decl list * sstruct_decl list

(* Pretty-printing functions *)

let rec string_of_sexpr (t, e) =
  "(" ^ string_of_typ t ^ " : " ^ (match e with
    SIntlit(l) -> string_of_int l
  | SFliteral(l) -> l
  | SCharlit(l) -> Char.escaped l
  | SStrlit(l) -> l
  | SBoolLit(true) -> "true"
  | SBoolLit(false) -> "false"
  | SArraylit(exp, i) -> "[" ^ String.concat ", " (List.map string_of_sexpr exp) ^ "] (length: " ^ (string_of_int i) ^ ")"
  | SArraccess(s, exp) -> (string_of_sexpr s) ^ "[" ^ (string_of_sexpr exp) ^ "]"
  | SId(s) -> s
  | SBinop(e1, o, e2) ->
      string_of_sexpr e1 ^ " " ^ string_of_op o ^ " " ^ string_of_sexpr e2
  | SUnop(o, e) -> string_of_uop o ^ string_of_sexpr e
  | SIncDec(e, i) -> string_of_sexpr e ^ string_of_incdec i
  | SAssign(v, e) -> string_of_sexpr v ^ " = " ^ string_of_sexpr e
  | SSretrieve(s, ele) -> string_of_sexpr s ^ "." ^ ele
  | SCall(f, el) ->
      f ^ "(" ^ String.concat ", " (List.map string_of_sexpr el) ^ ")"
  | SNoexpr -> ""
				  ) ^ ")"				     

let rec string_of_sstmt = function
    SBlock(_, stmts) ->
      "{\n" ^ String.concat "" (List.map string_of_sstmt stmts) ^ "}\n"
  | SExpr(expr) -> string_of_sexpr expr ^ ";\n";
  | SReturn(expr) -> "return " ^ string_of_sexpr expr ^ ";\n";
  | SIf(e, s, SBlock([], [])) ->
      "if (" ^ string_of_sexpr e ^ ")\n" ^ string_of_sstmt s
  | SIf(e, s1, s2) ->  "if (" ^ string_of_sexpr e ^ ")\n" ^
      string_of_sstmt s1 ^ "else\n" ^ string_of_sstmt s2
  | SFor(e1, e2, e3, s) ->
      "for (" ^ string_of_sexpr e1  ^ " ; " ^ string_of_sexpr e2 ^ " ; " ^
      string_of_sexpr e3  ^ ") " ^ string_of_sstmt s
  | SWhile(e, s) -> "while (" ^ string_of_sexpr e ^ ") " ^ string_of_sstmt s

let string_of_sfdecl sfdecl =
  string_of_typ sfdecl.styp ^ " " ^
  sfdecl.sfname ^ "(" ^ String.concat ", " (List.map snd sfdecl.sformals) ^
  ")\n{\n" ^
  String.concat "" (List.map string_of_vdecl sfdecl.slocals) ^
  String.concat "" (List.map string_of_sstmt sfdecl.sbody) ^
  "}\n"

let string_of_ssdecl ssdecl =
  "struct" ^ " " ^ ssdecl.ssname ^ "{\n" ^ 
  String.concat "" (List.map string_of_vdecl ssdecl.smembers) ^ "};\n"

let string_of_sprogram (vars, funcs, struc) =
  String.concat "" (List.map string_of_vdecl vars) ^ "\n" ^
  String.concat "\n" (List.map string_of_sfdecl funcs) ^ "\n" ^
  String.concat "\n" (List.map string_of_ssdecl struc)
